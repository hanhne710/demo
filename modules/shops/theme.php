...
function nv_template_view_all($array_data, $compare_id, $pages = '', $sort = 0, $viewtype = 'viewgrid')
{
    global $module_info, $lang_module, $module_name, $module_file, $pro_config, $array_wishlist_id, $global_array_shops_cat, $global_array_group, $my_head;

    $xtpl = new XTemplate('products.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
    $xtpl->assign('MODULE_NAME', $module_name);
    $xtpl->assign('LANG', $lang_module);
    $xtpl->assign('TEMPLATE', $module_info['template']);
    ...
    if (!empty($array_data)) {
        foreach ($array_data as $data_row) {
            if ($data_row['num_pro'] > 0) {
                ...
                $xtpl->assign('ID_CATALOG', $data_row['catid']);
...
function nv_template_detail($data_content, $data_unit, $data_others, $array_other_view, $content_comment, $compare_id, $popup, $idtemplates, $array_keyword)
{
    ...
    if (!empty($data_content)) {
        ...
        if ($pro_config['active_gift'] and !empty($data_content[NV_LANG_DATA . '_gift_content'])) {
        ...
        $xtpl->assign('table_custom_data', nv_custom_tpl('tab-thong-so-ky-thuat.tpl', $data_content['array_custom'], $data_content['array_custom_lang'], $idtemplates));
        
        $xtpl->assign('bodyhtml', $data_content[NV_LANG_DATA . '_bodytext']);
        ...
        foreach ($array_content as $value) {
            ...
            if ($pro_config['active_price'] == '1') {
                if ($value['showprice'] == '1' && !empty($value['product_price'])) {
                    $xtpl->assign('PRICE', $price);
                    if ($price['discount_percent'] > 0) {
                        $xtpl->parse('results.result.discounts');
                        $xtpl->parse('results.result.price.discounts');
                    } else {
                        $xtpl->parse('results.result.price.no_discounts');
                    }
                    $xtpl->parse('results.result.price');
                } else {
                    $xtpl->parse('results.result.contact');
                }
            }
            ...
            $xtpl->assign('CONTENT', $value['hometext']);
            $xtpl->assign('height', $pro_config['homeheight']);
            $xtpl->assign('width', $pro_config['homewidth']);
...
{
    global $home, $module_info, $lang_module, $lang_global, $module_name, $module_data, $module_file, $module_upload, $pro_config, $op, $compareid, $global_array_shops_cat;
    ...
    if (!empty($array_data)) {
        ...
        $xtpl->assign('NUM', ($home || $op != 'detail') ? 6 : 8);
...
function nv_template_viewcat($data_content, $compare_id, $pages, $sort = 0, $viewtype = 'viewgrid')
{
    ...
    if ($op != 'group' and $global_array_shops_cat[$data_content['id']]['viewcat'] == 'view_home_cat') {
        ...
        if ($sort == 1) {
            usort($data_content['data'], function ($item1, $item2) {
