...
if (strlen($key) >= NV_MIN_SEARCH_LENGTH) {
    ...
    $db->select('id, ' . NV_LANG_DATA . '_title, ' . NV_LANG_DATA . '_alias, listcatid, ' . NV_LANG_DATA . '_hometext, publtime, homeimgfile, homeimgthumb, product_price, money_unit, showprice, discount_id')->order($orderby)->limit($per_pages)->offset(($page - 1) * $per_page);
    ...
    while (list($id, $title, $alias, $listcatid, $hometext, $publtime, $homeimgfile, $homeimgthumb, $product_price, $money_unit, $showprice, $discount_id) = $result->fetch(3)) {
        if ($homeimgthumb == 1) {
            //image thumb
