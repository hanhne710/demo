...
if (!function_exists('nv_pro_catalogs')) {
    ...
    function nv_pro_catalogs($block_config)
    {
        ...
        foreach ($array_cat_shops as $cat) {
            if ($cat['parentid'] == 0) {
                if ($cat['inhome'] == '1') {
                    $html .= "<a title=\"" . $cat['title'] . "\" href=\"" . $cat['link'] . "\" class=\"cate-item\">" . (!empty($cat['image']) ? "<img src=\"" . NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module . '/' . $cat['image'] . "\">" : "") . nv_clean60($cat['title'], $cut_num) . "</a>\n";
                }
            }
        }
