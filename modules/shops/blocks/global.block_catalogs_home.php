...
if (!function_exists('nv_pro_catalogs_home')) {

    /**
     * nv_block_config_product_catalogs_home_blocks()
     *
     * @param mixed $module
     ...
     * @return
     */
    function nv_block_config_product_catalogs_home_blocks($module, $data_block, $lang_block)
    {
    ...
    /**
     * nv_block_config_product_catalogs_home_blocks_submit()
     *
     * @param mixed $module
     * @param mixed $lang_block
     * @return
     */
    function nv_block_config_product_catalogs_home_blocks_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    /**
     * nv_pro_catalogs_home()
     *
     * @param mixed $block_config
     * @return
     */
    function nv_pro_catalogs_home($block_config)
    {
        ...
        $block_tpl_name = "block.catalogs_home.tpl";
        ...
        if ($module != $module_name) {
            $sql = "SELECT catid, parentid, lev, " . NV_LANG_DATA . "_title AS title, " . NV_LANG_DATA . "_alias AS alias, viewcat, numsubcat, subcatid, numlinks, " . NV_LANG_DATA . "_description AS description, image, inhome, " . NV_LANG_DATA . "_keywords AS keywords, groups_view FROM " . $db_config['prefix'] . "_" . $mod_data . "_catalogs ORDER BY sort ASC";
        ...
        $html = "<ul class=\"home-cates\">";

        foreach ($array_cat_shops as $cat) {
            if ($cat['parentid'] == 0) {
                if ($cat['inhome'] == '1') {
                    $html .= "<li><a title=\"" . $cat['title'] . "\" href=\"" . $cat['link'] . "\" class=\"cate-item\">" . nv_clean60($cat['title'], $cut_num) . "</a>\n";
                    if (!empty($cat['subcatid'])) {
                        $html .= html_viewsubcat($cat['subcatid'], $block_config);
                    }
                    $html .= "</li>\n";
                }
            }
    ...
    function html_viewsubcat($list_sub, $block_config)
    {
        ...
        if (empty($list_sub)) {
            ...
            $html = "<ul class=\"sub-home-cates\">\n";
            $list = explode(",", $list_sub);
            foreach ($list as $catid) {
                if ($array_cat_shops[$catid]['inhome'] == '1') {
                    $html .= "<li class=\"dropdown\">\n";
                    $html .= "<span><a title=\"" . $array_cat_shops[$catid]['title'] . "\" href=\"" . $array_cat_shops[$catid]['link'] . "\">" . nv_clean60($array_cat_shops[$catid]['title'], $cut_num) . "</a></span>\n";
    ...
    function html_viewsub2($list_sub, $block_config)
    {
        ...
        if (empty($list_sub)) {
            ...
            $html = "<ul class=\"sub-home-cates-min\">\n";
            $list = explode(",", $list_sub);
            foreach ($list as $catid) {
                if ($array_cat_shops[$catid]['inhome'] == '1') {
                    $html .= "<li>\n";
...
if (defined('NV_SYSTEM')) {
    $content = nv_pro_catalogs_home($block_config);
}
