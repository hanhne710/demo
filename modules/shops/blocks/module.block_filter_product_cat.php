...
if (!function_exists('nv_filter_product_cat')) {
    ...
    function nv_filter_product_cat($block_config)
    {
        ...
        $result = $db->query('SELECT t1.groupid FROM ' . $db_config['prefix'] . '_' . $site_mods[$module]['module_data'] . '_group_cateid t1 INNER JOIN ' . $db_config['prefix'] . '_' . $site_mods[$module]['module_data'] . '_group t2 ON t1.groupid = t2.groupid WHERE t1.cateid = ' . $catid . ' ORDER BY t2.weight');
        $i = 0;
