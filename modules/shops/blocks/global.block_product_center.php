...
if (!nv_function_exists('nv_global_product_center')) {
    ...
    function nv_global_product_center($block_config)
    {
        ...
        $db->sqlreset()
            ->select('*')
            ->from($db_config['prefix'] . '_' . $mod_data . '_block_cat')
            ->where('bid= ' . $block_config['blockid']);
        $block_info = $db->query($db->sql())->fetch();

        $block_info['title'] = $block_info[NV_LANG_DATA . '_title'];
        $block_info['link'] = NV_BASE_SITEURL . 'blockcat/' . $block_info[NV_LANG_DATA . '_alias'] . '/';
        $block_info['image'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $site_mods[$module]['module_upload'] . '/' . $block_info['image'];
        $xtpl->assign('BLOCK_INFO', $block_info);

        $db->sqlreset()
            ->select('t1.id, t1.listcatid, t1.' . NV_LANG_DATA . '_title AS title, t1.' . NV_LANG_DATA . '_alias AS alias, t1.' . NV_LANG_DATA . '_hometext AS hometext, t1.homeimgfile, t1.homeimgthumb , t1.homeimgalt, t1.showprice, t1.product_price, t1.product_number')
        ...
        foreach ($list as $row) {
            ...
            if ($pro_config['active_order'] == '1' and $pro_config['active_order_non_detail'] == '1') {
                if ($row['showprice'] == '1' && !empty($row['product_price'])) {
