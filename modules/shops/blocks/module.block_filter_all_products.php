...
if (!function_exists('nv_filter_all_products')) {
    /**
     * nv_block_config_filter_all_products()
     *
     * @param mixed $module
     ...
     * @return
     */
    function nv_block_config_filter_all_products($module, $data_block, $lang_block)
    {
    ...
    /**
     * nv_block_config_filter_all_products_submit()
     *
     * @param mixed $module
     * @param mixed $lang_block
     * @return
     */
    function nv_block_config_filter_all_products_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = [];
        $return['error'] = [];
    ...
    /**
     * nv_filter_all_products()
     *
     * @return
     */
    function nv_filter_all_products($block_config)
    {
        global $module_name, $lang_module, $module_info, $site_mods, $module_file, $module_upload, $db, $module_data, $db_config, $id, $pro_config, $global_config, $global_array_group, $global_array_shops_cat, $nv_Request, $array_id_group, $op;
        ...
        $xtpl = new XTemplate('block.filter_all_products.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
        $xtpl->assign('LANG', $lang_module);
        $xtpl->assign('REWRITE_ENABLE', $global_config['rewrite_enable'] ? 'true' : 'false');
        $xtpl->assign('AJAX_URL', nv_url_rewrite(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $site_mods[$module_name]['alias']['products'], true));
        $xtpl->assign('LINK_URL', rtrim(nv_url_rewrite(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $site_mods[$module_name]['alias']['products'], true), '/'));

        $result = $db->query('SELECT t1.groupid FROM ' . $db_config['prefix'] . '_' . $site_mods[$module]['module_data'] . '_group_cateid t1 INNER JOIN ' . $db_config['prefix'] . '_' . $site_mods[$module]['module_data'] . '_group t2 ON t1.groupid = t2.groupid ORDER BY t2.weight');
        $i = 0;
        ...
        foreach (array_unique($array_grid) as $groupid) {
...
if (defined('NV_SYSTEM')) {
    $module = $block_config['module'];
    $content = nv_filter_all_products($block_config);
}
