...
// Group custom
$groups_list = array();
$result = $db->query('SELECT group_id, alias as title, idsite FROM ' . NV_GROUPS_GLOBALTABLE . ' WHERE group_id NOT IN ( 4, 5, 6 ) AND (idsite = ' . $global_config['idsite'] . ' OR (idsite =0 AND siteus = 1)) ORDER BY idsite, weight');
...
if ($savesetting == 1) {
    ...
    $gift_default = $nv_Request->get_string('gift_default', 'post', '');
    $data['gift_default'] = defined('NV_EDITOR') ? nv_nl2br($gift_default, '') : nv_nl2br(nv_htmlspecialchars(strip_tags($gift_default)), '<br />');
...
$data['gift_default'] = htmlspecialchars(nv_editor_br2nl($data['gift_default']));
if (defined('NV_EDITOR') and function_exists('nv_aleditor')) {
    $edits = nv_aleditor('gift_default', '100%', '150px', $data['gift_default']);
} else {
    $edits = "<textarea style=\"width: 100%\" name=\"gift_default\" id=\"gift_default\" cols=\"20\" rows=\"15\">" . $data['gift_default'] . "</textarea>";
}
$xtpl->assign('edit_gift_default', $edits);
