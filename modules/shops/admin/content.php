...
$rowcontent = [
    ...
    'files' => [],
    'files_old' => [],
    'gift_content' => $module_config[$module_name]['gift_default'],
...
if ($nv_Request->get_int('save', 'post') == 1) {
    ...
    $gift_content = $nv_Request->get_string('gift_content', 'post', '');
    $rowcontent['gift_content'] = defined('NV_EDITOR') ? nv_nl2br($gift_content, '') : nv_nl2br(nv_htmlspecialchars(strip_tags($gift_content)), '<br />');
...
$rowcontent['gift_content'] = htmlspecialchars(nv_editor_br2nl($rowcontent['gift_content']));
if (defined('NV_EDITOR') and function_exists('nv_aleditor')) {
    $edits = nv_aleditor('gift_content', '100%', '150px', $rowcontent['gift_content']);
} else {
    $edits = "<textarea style=\"width: 100%\" name=\"gift_content\" id=\"gift_content\" cols=\"20\" rows=\"15\">" . $rowcontent['gift_content'] . "</textarea>";
}
$xtpl->assign('edit_gift_content', $edits);
